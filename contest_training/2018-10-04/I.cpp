...
int work(int u,int v)
{
    ...
    if(dep[u]<dep[v])
        up(v,dep[v]-dep[u],res);
    else
        up(u,dep[u]-dep[v],res);
...
int main()
{
    ...
    while(~scanf("%d%d",&n,&m))
    {
        ...
        for(int i=1; i<=m; i++)
            e1[i].u=read(),e1[i].v=read(),e1[i].w=read();//scanf("%d%d%d",&e1[i].u,&e1[i].v,&e1[i].w);
        ...
        for(int i=1; i<=m; i++)
        {
            ...
            {
                mp[edge1(e1[i].u,e1[i].v,0)]=e1[i].w;
                mp[edge1(e1[i].v,e1[i].u,0)]=e1[i].w;
            }
        }
