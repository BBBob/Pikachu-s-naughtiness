...
## E 无效位置
### 题意
给一个数组 $a$ ，有 $n$ 次操作，每次操作会使一个位置无效。一个区间的权值定义为这个区间里选出一些数的异或和的最大值。求在每次操作前，所有不包含无效位置的区间的权值的最大值。
### 题解
考虑离线逆向操作（时光倒流），将其从正向删去节点变成逆向添加节点。为每个区间开一个线性基，对于当前添加的点， 如果相邻的点存在，就将其和相邻的点的线性基合并。然后利用当前线性基计算子集异或最大值，并更新答案。
线性基合并时间复杂度： $O(\log ^2 n)$ 
总时间复杂度： $O(n \log ^2 n)$
## F 细胞
